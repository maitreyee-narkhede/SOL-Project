...
def payment():
    ...
    insert = "insert into cardinfo(id, accountno, phoneno, cardno, cvv) values(%d, %s, %d, %d, %s);"
    username = request.form["username"]
    ...
    if account=='' or card=='' or phone=='' or cvv=='':
        ...
        if int(account)<100000000 or int(account)>=1000000000000000:
            ...
            return jsonify(object)
        elif int(card)<100000 or int(card)>100000000:
            object = {'code': 'false',
    ...
    result = cur.fetchone()
    cur.execute(insert % (result[0], encrypt(account), int(phone), int(card), encrypt(cvv)))
    object = {'code': 'true',
              'text': 'Authentication successful'}
    return jsonify(object)
...
@cross_origin()
def pin_enc():
    add_pin = "update cardinfo set pin='%s' where cardno='%d';"
    ...
    cur.execute("rollback")
    cur.execute(add_pin % (encrypt(pin), int(card)))
    object = {'code': 'true',
              'text': 'Authentication successful'}
    return jsonify(object)
